"""The environment failure a real run actually hit, caught before the download.
...
That is a **baseline** concern and did not retire with the heterogeneous
portfolio: `Qwen/Qwen3.5-4B` declares `model_type: qwen3_5` exactly as the
retired 9B did. So the `hf` extra pins `transformers>=5.2.0` and the runner
checks it before the first `build_runtime`.
...
HF_BLOCK = {"backend": "huggingface", "model_id": "Qwen/Qwen3.5-4B"}
...
def test_a_neural_profile_is_checked() -> None:
    assert needs_transformers(STUB_BLOCK, HF_BLOCK) is True
    report = huggingface_runtime_report(STUB_BLOCK, HF_BLOCK,
...
def test_the_gate_raises_and_names_the_fix() -> None:
    with pytest.raises(RuntimeError, match="cannot load the declared models"):
        require_huggingface_runtime(HF_BLOCK, version_string="4.57.6")


def test_the_gate_is_silent_on_a_ready_environment() -> None:
    require_huggingface_runtime(HF_BLOCK, version_string="5.2.0")
...
def test_pyproject_pins_a_transformers_that_can_load_qwen3_5() -> None:
    body = (REPO_ROOT / "pyproject.toml").read_text()
    assert '"transformers>=5.2.0"' in body
...
def test_the_baseline_verifier_is_what_needs_the_floor() -> None:
    """Not a portfolio concern: the baseline's own verifier is `qwen3_5`."""
